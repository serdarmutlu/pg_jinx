...
//	List		*options_list = untransformRelOptions(PG_GETARG_DATUM(0));
    printf("pg_jinx.c - fdw_validator begin\n");
...
//    char *classname = NULL;
    ...
    if (catalog == ForeignDataWrapperRelationId) {
        printf("pg_jinx.c - fdw_validator 1\n");
        PG_RETURN_VOID();
    };
...
/*	foreach(cell, options_list) {
    ...
    */
    printf("pg_jinx.c - fdw_validator 2\n");
...
Datum fdw_handler(PG_FUNCTION_ARGS) {
    ...
    printf("pg_jinx.c - fdw_handler begin\n");
    ...
    printf("pg_jinx.c - fdw_handler 1\n");
...
Datum inline_handler(PG_FUNCTION_ARGS) {
    printf("pg_jinx.c - inline_handler begin\n");
    ...
    jarray argt = (*env)->NewIntArray(env, 0);
 	...
 	jobject result = (*env)->CallObjectMethod(env, fn, functionInvoke, src, paramNames, argt, NULL, paramNames);
  	check_exception("%s\ncalling inline method: %s", codeblock->source_text);	
        printf("pg_jinx.c - inline_handler 1\n");
...
HeapTuple handleRecordD(jarray result, TupleDesc tupdesc) {
    //printf("pg_jinx.c - handleRecordD begin\n");
...
//        	pfree(dvalues);   ??
    ...
    //printf("pg_jinx.c - handleRecordD 1\n");
...
static Datum handleRecord(jarray result, PG_FUNCTION_ARGS) {
    ...
    printf("pg_jinx.c - handleRecord begin\n");
...
static Oid resolveDomain(Oid type) {
	...
	if(typeStruct->typbasetype != 0) {
        ...
        printf("pg_jinx.c - resolveDomain 1\n");
...
*/	
    ReleaseSysCache(typeTup);
    printf("pg_jinx.c - resolveDomain 2\n");
...
static jobject callJavaMethod(jobject fn, HeapTuple procTup, PG_FUNCTION_ARGS) {
  ...
  // look at how this is done in plpy_procedure.c  (plpython)
    printf("pg_jinx.c - callJavaMethod begin\n");
...
Datum internalCallHandler(bool trusted, PG_FUNCTION_ARGS) {
    ...
    printf("pg_jinx.c - internalCallHandler begin\n");
    ...
    printf("pg_jinx.c - internalCallHandler 1\n");

    if (multicall) {
        if (SRF_IS_FIRSTCALL()) {
            startSRF(fn, procTup, fcinfo);
        }
        printf("pg_jinx.c - internalCallHandler 2\n");
        ...
        if (result == NULL) {
            fcinfo->isnull = true;
            printf("pg_jinx.c - internalCallHandler 3\n");

            return NULL;
        }
	    if (rettype == RECORDOID) {         printf("pg_jinx.c - internalCallHandler 4\n"); return handleRecord(result, fcinfo); }
                printf("pg_jinx.c - internalCallHandler 5\n");
