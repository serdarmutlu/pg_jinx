...
static void SIGINTInterruptCheckProcess( javaFdwExecutionState *state) {
    //printf("pg_jinx_fdw.c - SIGINTInterruptCheckProcess begin\n");
    ...
    //printf("pg_jinx_fdw.c - SIGINTInterruptCheckProcess end\n");
}
...
static javaFdwExecutionState* javaGetOptions(Oid foreigntableid) {
    ...
    char *classname = NULL;
    printf("pg_jinx_fdw.c - javaGetOptions begin\n");
	...
	// (EState *estate, ResultRelInfo *rinfo) 
        ...
        printf("pg_jinx_fdw.c - javaGetOptions end\n");
...
FdwPlan*javaPlanForeignScan(Oid foreigntableid, PlannerInfo *root, RelOptInfo *baserel) {
    ...
    printf("pg_jinx_fdw.c - javaPlanForeignScan begin\n");
    ...
    printf("pg_jinx_fdw.c - javaPlanForeignScan end\n");
    return (fdwplan);
}
#endif
...
void javaExplainForeignScan(ForeignScanState *node, ExplainState *es) {
    ...
    printf("pg_jinx_fdw.c - javaExplainForeignScan begin\n");

    javaFdwExecutionState   *state = (javaFdwExecutionState *)node->fdw_state;
    SIGINTInterruptCheckProcess(state);

    printf("pg_jinx_fdw.c - javaExplainForeignScan - javaIterateForeignScan methodid:%d\n",state-> iterateForeignScan);

    (*env)->CallObjectMethod(env, state->java_call, state->explainForeignScan);
    CHECK_EXCEPTION("%s\ncalling explainForeignScan","unused");
    printf("pg_jinx_fdw.c - javaExplainForeignScan end\n");
}
...
void javaBeginForeignScan(ForeignScanState *node, int eflags) {
    ...
    printf("pg_jinx_fdw.c - javaBeginForeignScan begin\n");
...
// there is no way to get the private data from the GetForeignXXX calls
	...
	// if (id_numberofcolumns == NULL) { elog(ERROR, "id_numberofcolumns is NULL"); }
            printf("pg_jinx_fdw.c - javaBeginForeignScan end\n");
...
TupleTableSlot* javaIterateForeignScan(ForeignScanState *node) {
    ...
    //printf("pg_jinx_fdw.c - javaIterateForeignScan begin\n");
    ...
    //printf("pg_jinx_fdw.c - javaIterateForeignScan end\n");
...
void javaEndForeignScan(ForeignScanState *node) {
    printf("pg_jinx_fdw.c - javaEndForeignScan begin\n");
    javaFdwExecutionState *state = (javaFdwExecutionState *)node->fdw_state;
	SIGINTInterruptCheckProcess(state);
    ...
    printf("pg_jinx_fdw.c - javaEndForeignScan end\n");
}

void javaReScanForeignScan(ForeignScanState *node) {
    printf("pg_jinx_fdw.c - javaReScanForeignScan begin\n");
	SIGINTInterruptCheckProcess(node->fdw_state);
    printf("pg_jinx_fdw.c - javaReScanForeignScan end\n");
}
...
void javaGetForeignPaths(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid) {
	Cost 		startup_cost = 0;
	Cost 		total_cost = 0;
    printf("pg_jinx_fdw.c - javaGetForeignPaths begin\n");
    ...
    printf("pg_jinx_fdw.c - javaGetForeignPaths end\n");            
}
...
{
    printf("pg_jinx_fdw.c - javaGetForeignPlan start\n");
	...
	return (make_foreignscan(tlist, scan_clauses, scan_relid, NIL, baserel->fdw_private
    ...
    printf("pg_jinx_fdw.c - javaGetForeignPlan end\n");
}
...
void javaGetForeignRelSize(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid) {
    ...
    printf("pg_jinx_fdw.c - javaGetForeignRelSize begin\n");
    ...
    #else
	List	   *targetcolumns = pull_var_clause((Node *) node->clause, PVC_RECURSE_AGGREGATES , PVC_RECURSE_PLACEHOLDERS);
    #endif		columns = list_union(columns, targetcolumns);
	}
    ...
    baserel -> rows = rx[0];
    #if PG_VERSION_NUM >= 90500
        baserel -> reltarget -> width = rx[1]; // number of bytes per row
    #else
    ...
    printf("pg_jinx_fdw.c - javaGetForeignRelSize end\n");
